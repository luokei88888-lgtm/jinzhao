...
产物在 `src-tauri/target/release/bundle/nsis/`，文件名形如 `今朝_0.1.0_x64-setup.exe`，实测 2.16 MB。可执行文件本身叫 `jinzhao.exe`（沿用 Cargo 包名），安装后开始菜单和标题栏显示的都是「今朝」。

当前未做代码签名，安装时 SmartScreen 会拦截，需要自行购买证书后重打。

提交到 `main` 会触发 `.github/workflows/ci.yml`，在 Windows 上跑前端测试、类型检查、前端构建和 Rust 测试。
...
- 打包验证：`npm run tauri build -- --bundles nsis` 成功产出 `今朝_0.1.0_x64-setup.exe`，2.16 MB。安装包尚未在本机执行安装（安装会写入系统，需要事先确认）
...
- 节假日表依赖第三方接口（timor.tech 与 holiday-cn），官方只发布通知没有 API。两个源都不可用或次年数据未发布时界面会隐藏休班角标；需要强制刷新时删掉 `%APPDATA%\com.jinzhao.app\holidays-<年份>.json` 再启动
